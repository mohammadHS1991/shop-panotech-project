...
export const POST = async (request) => {
  ...
  if (API_KEY === process.env.NEXT_PUBLIC_API_KEY) {
    try {
      ...
      if (!user) {
        return NextResponse.json("توکن نامعتبر", {
          status: 400,
        });
      }
      ...
      const mailToAdmin = await mailer(
        ...
        "panotech | auth",
        `<h1>api auth verify-token post error, status:500</h1>
        <h2>${err}</h2>`
      );
      logger.error(`api auth verify-token post error, status:500/ ${err}`);
    ...
    const mailToAdmin = await mailer(
      process.env.ADMIN_EMAIL,
      "panotech | auth",
      `<h1>api auth verify-token post error, status:403</h1>`
    );
    logger.error(`api auth verify-token post error, status:403`);
