...
import { Question } from "../../[lang]/models";
...
export const GET = async () => {
  ...
  if (API_KEY === process.env.GET_API_KEY) {
    try {
      ...
      logger.error(`api questions get error, status:500/ ${err} `);

      return Response.json("db error", { status: 500 });
    }
  } else {
    logger.error(`api questions get error, status:403`);
  ...
  if (API_KEY === process.env.NEXT_PUBLIC_API_KEY) {
    try {
      ...
      if (captchaResponse.success) {
        ...
        try {
          const validatedQuestion = await Question.questionValidation({
            firstName,
            lastName,
            email,
            job,
        ...
        const mailBody = `
            <div dir="rtl">
                <h2>سوال کننده: ${firstName} ${lastName}</h2>
            ...
            `;
        const subject = "پانوتک | سوال جدید";
      ...
      const mailToAdmin = await mailer(
        ...
        "panotech | questions",
        `<h1>api questions post error, status:500</h1>
        <h2>${err}</h2>`
      );
      logger.error(`api questions post error, status:500/ ${err} `);
    ...
    const mailToAdmin = await mailer(
      process.env.ADMIN_EMAIL,
      "panotech | questions",
      `<h1>api questions post error, status:403</h1>`
    );
    logger.error(`api questions post error, status:403`);
