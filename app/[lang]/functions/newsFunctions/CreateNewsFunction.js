const CreateNewsFunction = async (data) => {
  try {
    const res = await fetch(
      process.env.NEXT_PUBLIC_PANOTECH_BASE_URL + `/api/blogs/news`,
      {
        method: "POST",
        cache: "no-cache",
    ...
    if (res.status === 409) {
      throw new Error("این خبر قبلاً ثبت شده است");
    }
...
export default CreateNewsFunction;
