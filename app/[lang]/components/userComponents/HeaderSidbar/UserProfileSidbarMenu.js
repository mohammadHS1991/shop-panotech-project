...
import { userProfileNavLink } from "@/app/[lang]/data/data";
...
const UserProfileSidbarMenu = ({ lang }) => {
  ...
  return (
    <nav className="flex h-full pb-28">
      ...
      >
        {userProfileNavLink?.map((link, index) => (
          <Tooltip
            key={index}
            color="success"
            ...
            className={`${
              url == `/${lang}${link.href}`
                ...
                : ""
                            ...
                            py-2 hidden lg:flex justify-center `}
          >
            {link.title[lang]}
          </Link>
        ))}
...
export default UserProfileSidbarMenu;
